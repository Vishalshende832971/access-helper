...
try:
    if os.getenv("INTEGRATION_TEST") == "TRUE":
        raise google.auth.exceptions.DefaultCredentialsError("Bypassing GCP logging for integration tests")
    _, project_id = google.auth.default()
    logging_client = google_cloud_logging.Client()
    logger = logging_client.logger(__name__)
except Exception as e:
    std_logging.warning(f"Failed to initialize Google Cloud Logging, using standard logging: {e}")
...
app.title = "access-helper"
app.description = "API for interacting with the Agent access-helper"
